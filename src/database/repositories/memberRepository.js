...
class MemberRepository extends AbstractRepository {
  ...
  async create(data, options) {
    const record = await models.member.create(
      {
  ...
  async update(id, data, options) {
    let record = await models.member.findByPk(id, {
      transaction: AbstractRepository.getTransaction(
        options,
      ),
    });
    ...
    data.activedBy = record.activedById;
  ...
  async findById(id, options) {
    const record = await models.member.findByPk(id, {
  ...
  async findByStripeCustomerId(stripeCustomerId, options) {
    const record = await models.member.findOne(
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.membershipYearRange) {
        sequelizeFilter.appendRange(
          'membershipYear',
          filter.membershipYearRange,
        );
      }
      ...
      if (filter.subscriptionRenewsAtRange) {
        sequelizeFilter.appendRange(
          'subscriptionRenewsAt',
          filter.subscriptionRenewsAtRange,
        );
      }
    }
    ...
    let {
      rows,
      count,
    } = await models.member.findAndCountAll({
  ...
  async findAllAutocomplete(query, limit) {
    ...
    if (query) {
      ...
      filter.appendIlike('name', query, 'member');
    }

    const records = await models.member.findAll({
      attributes: ['id', 'name'],
