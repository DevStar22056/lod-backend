...
class SkuRepository extends AbstractRepository {
  ...
  async create(data, options) {
    const record = await models.sku.create(
      {
  ...
  async update(id, data, options) {
    let record = await models.sku.findByPk(id, {
      transaction: AbstractRepository.getTransaction(
        options,
      ),
    });
  ...
  async findById(id, options) {
    const record = await models.sku.findByPk(
      id,
      {
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.name) {
        sequelizeFilter.appendIlike('name', filter.name, 'sku');
      }
    ...
    let { rows, count } = await models.sku.findAndCountAll({
  ...
  async findAllAutocomplete(query, limit) {
    ...
    if (query) {
      ...
      filter.appendIlike('name', query, 'sku');
    }

    const records = await models.sku.findAll({
      attributes: ['id', 'name'],
