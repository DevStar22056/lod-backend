...
class SubscriptionPaymentRepository extends AbstractRepository {
  constructor() {
    super();
  ...
  async create(data, options) {
    const record = await models.subscriptionPayment.create(
      {
  ...
  async update(id, data, options) {
    let record = await models.subscriptionPayment.findByPk(
      id,
      {
        transaction: AbstractRepository.getTransaction(
          options,
  ...
  async findById(id, options) {
    const record = await models.subscriptionPayment.findByPk(
      id,
      {
  ...
  async count(filter, options) {
    return models.subscriptionPayment.count(
      {
        where: filter,
      },
      {
  ...
  async _createOrUpdateFiles(record, data, options) {
    for (const field of this.fileAttributes) {
      await FileRepository.replaceRelationFiles(
        {
          belongsTo: models.subscriptionPayment.getTableName(),
  ...
  async findAndCountAll(
    ...
    let {
      rows,
      count,
    } = await models.subscriptionPayment.findAndCountAll({
  ...
  async findAllAutocomplete(query, limit) {
    ...
    const records = await models.subscriptionPayment.findAll(
      {
        attributes: ['id', 'stripeInvoice'],
        where: filter.getWhere(),
        limit: limit ? Number(limit) : undefined,
        orderBy: [['stripeInvoice', 'ASC']],
      },
    );
